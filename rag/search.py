...
@traceable()
async def search_drinks_by_brand(brand: str, fetch_k: int = 20) -> list[any]:
...
@traceable()
async def search_drinks_hybrid(brand: str, query_text: str, fetch_k: int = 10) -> list[any]:
    ...
    async with get_pool().connection() as conn:
        async with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
            await cur.execute("""
                ...
                ORDER BY rrf_score DESC
                LIMIT %(fetch_k)s;
            """, {"embedding": str(query_vector), "query": query_text, "fetch_k": fetch_k, "brand": brand})
            return await cur.fetchall()
